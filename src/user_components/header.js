...
const handleSignOut = () => {
  ...
  return (
    <header className=' w-full h-14 bg-whitw shadow-2xl opacity-4  text-black text-center'>
      ...
      <div className='text-2xl font-extrabold  font-mono cursor-pointer'onClick={()=>handleNavigate('/user/dashbord')}>Dashboard</div>
      <div className='mr-4 flex justify-between w-20'>
        <div className=' text-3xl cursor-pointer' onClick={()=>handleNavigate('/user/setting')}>{<IoSettingsSharp/>}</div>
